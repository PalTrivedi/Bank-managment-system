/*Bank software
Q1)admin or customer.
if ADMIN-->enter pass(predefined)-->MENU
1)Monthly interest customerwise
2)Check acc with proper bal.(validation)
3)check acc with low bal.(validation)
4)(Block) Dormat acc with repeat low bal.
5)All details.(name,contactno,savings/current,bal,interest)
6)EXIT and back to main menu.

if CUSTOMER-->
1)Sign up(name,10 digit mobile no.(validation), pass, confirm pass,ask for acc type) 
2)Sign in-->mob,pass-->(dropdown menu)-->
check bal,withdrawal,deposit,interest(compound) calculation(for savings,1year only),acc statement(all trasaction, deposit,withdraw
,balance,interest is deposited),EXIT and back to main MENU*/
#include<iostream>
#include<string.h>
#include<fstream>
#include<conio.h>
